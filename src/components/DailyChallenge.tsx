...
export function DailyChallenge({ question, options, chapterTitle }: Props) {
  ...
  const optionCls = (label: string) => {
    ...
    if (!isAnswered) return base + "border-slate-200 hover:border-sky-300 hover:bg-sky-50 text-slate-700 cursor-pointer";
    if (label === correct?.label) return base + "border-green-400 bg-green-50 text-green-800 font-medium";
    if (label === selected) return base + "border-red-300 bg-red-50 text-red-700";
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6">
            ...
            <p className="text-sm font-semibold text-slate-900 mb-4 leading-snug">{question.stem}</p>
